...
1. **Preferencia por Micro-transacciones Ágiles:** Se asume que los usuarios peruanos prefieren la inmediatez al adquirir libros y estarían dispuestos a pre-cargar fondos en la plataforma para evitar el tedio de las transferencias manuales repetitivas, confiando en que sus fondos estarán disponibles para compras futuras.
...
5. **Los GIFs Elevan la Calidad Percibida de los Debates Literarios:** Se asume que el formato GIF no trivializa las discusiones sobre libros sino que las enriquece, permitiendo expresar reacciones emocionales ante un pasaje o personaje de forma más precisa que el texto plano.
...
8. **El Rechazo Rápido No Degrada la Calidad del Algoritmo:** Se asume que aunque el swipe acelera la decisión de descarte, los usuarios seguirán rechazando libros que genuinamente no les interesan (por portada, género o autor) y no libros potencialmente valiosos, manteniendo la calidad de las señales de preferencia.
9. **La Vitrina Pública Incentiva la Exploración del Catálogo por Terceros:** Se asume que cuando un usuario visita el perfil de otro lector y ve su historial de libros leídos, esto actúa como una recomendación social implícita que lo lleva a explorar títulos que de otro modo no habría descubierto en el catálogo.
10. **El Saldo Visible Reduce la Fricción de Decisión en el Catálogo:** Se asume que mostrar el saldo disponible en la Billetera Livria directamente en la pantalla del catálogo hace que el usuario perciba los precios de los libros como un gasto de fondos ya comprometidos (y por tanto psicológicamente "gastados"), reduciendo la fricción de añadir al carrito.

### Knowledge Gaps

1. **Nivel Real de Confianza en Fondos Cautivos:** Desconocemos si el usuario peruano promedio confía lo suficiente en una plataforma literaria de reciente lanzamiento como para inmovilizar dinero en una billetera digital interna, o si prefiere el control de pagar únicamente cuando confirma cada compra.
...
3. **Calidad vs. Cantidad en el Tinder Literario:** No sabemos si al reducir el tiempo de decisión mediante swipe, los usuarios comenzarán a descartar libros valiosos basándose únicamente en la portada o el título, deteriorando la calidad de las señales de preferencia que alimentan el algoritmo.
4. **Impacto Real de la Vitrina Pública en el Comportamiento:** Carecemos de datos sobre si la habilitación de un historial de lectura público efectivamente incrementa las visitas entre perfiles, la tasa de unión a nuevas comunidades o las compras inspiradas en el catálogo de otro usuario.
5. **Riesgo de Saturación Visual por Multimedia en Foros:** No tenemos certeza sobre si el soporte de GIFs en los hilos de discusión derivará en conversaciones más ricas o en un caos visual que desvíe la atención de los temas literarios hacia el contenido multimedia por sí mismo.
6. **Monto Promedio Dispuesto a Pre-cargar:** Desconocemos cuál es el monto de recarga inicial que el usuario peruano consideraría razonable para la Billetera Livria — si sería equivalente al precio de un libro (~S/ 40), de varios (~S/ 100) o si preferiría montos mínimos simbólicos (~S/ 20).
7. **Punto Exacto de Abandono en el CCI:** Aunque los eventos del Experimento 2 (`view_payment_instructions` vs. `upload_payment_receipt`) miden el abandono en el bloque de pago, aún desconocemos si el mayor abandono ocurre en el paso de realizar la transferencia bancaria o en el de subir el comprobante, lo cual determina qué parte del flujo optimizar primero.
...
9. **Impacto del Swipe de Rechazo en la Percepción de Marca:** Desconocemos si la mecánica de descartar libros mediante swipe izquierdo puede generar una asociación negativa con la plataforma, especialmente si el usuario descarta libros de autores que luego ve destacados en otras secciones de la app.
10. **Comparación Social Negativa en la Vitrina Literaria:** Carecemos de datos sobre si exponer públicamente cuántos libros ha leído cada usuario podría generar dinámicas de comparación negativa entre lectores ocasionales y lectores intensivos, afectando la retención del segmento de menor frecuencia de lectura.
...
5. **Comentarios y Posts con GIFs e Imágenes en Comunidades:** Habilitar el soporte de GIFs animados —además de las imágenes estáticas ya existentes— en publicaciones y comentarios de los clubes de lectura, midiendo si este formato eleva la frecuencia de interacción y la riqueza expresiva de los debates literarios.
6. **Notificación Push de Carrito Abandonado:** Enviar una notificación push personalizada cuando un usuario haya iniciado el checkout (`begin_checkout`) pero no haya completado la compra en las siguientes 24 horas, con un mensaje contextual que mencione el título del libro pendiente.
7. **Indicador de Saldo en Pantalla de Catálogo:** Mostrar el saldo disponible en la Billetera Livria de forma persistente y visible en la barra superior del catálogo de libros, reduciendo la fricción de recordar cuánto se tiene disponible antes de añadir al carrito.
...
9. **Selector de Idioma Accesible desde el Login:** Incorporar un toggle de idioma (ES / EN) visible directamente en la pantalla de login, antes de que el usuario acceda a la app, para que la experiencia comience en el idioma correcto desde el primer contacto.
10. **Vista Previa de Swipe en Onboarding:** Introducir una pantalla de onboarding que demuestre la mecánica del Tinder Literario con 3 tarjetas de ejemplo antes de que el usuario acceda al catálogo por primera vez, reduciendo la curva de aprendizaje de la nueva interacción.
...
5. **Revitalización del Engagement Social con GIFs:** La incorporación de GIFs animados en los posts y comentarios de las comunidades romperá la monotonía del texto plano y aumentará la frecuencia de publicaciones por usuario activo, elevando la métrica `post_in_community` medida en el Experimento 6.
6. **El CCI es el Principal Punto de Abandono del Funnel de Ventas:** El ratio entre `add_payment_info` y `purchase` instrumentado en el Experimento 2 confirma que la mayor pérdida de usuarios ocurre en el paso de transferencia manual, y no en la selección del libro ni en el ingreso de datos del destinatario.
7. **Los Usuarios que Publican Reseñas tienen Mayor Retención a 30 Días:** Los usuarios que han posteado al menos una reseña en `SingleBookView` tienen una probabilidad significativamente mayor de retornar a la plataforma en los siguientes 30 días, lo que valida la importancia de incentivar y visibilizar este comportamiento.
...
10. **Los Usuarios que Pre-cargan Fondos Realizan Más Compras por Sesión:** Se afirma que la disponibilidad inmediata de fondos en la billetera elimina la fricción de decisión en el momento de la compra impulsiva, resultando en una mayor cantidad de ítems adquiridos por sesión en comparación con el flujo de CCI individual.
